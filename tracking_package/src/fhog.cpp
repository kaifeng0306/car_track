  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
    ...
    if( softBin<0 && softBin%2==0 ) {
      ...
      // if( softBin<0 ) for( ; ; y++ ) {
      //   yb0 = (int) yb; if(yb0>=hb-1) break; GHinit;
      //   if(hasLf) { H0[O0[y]] += M0[y]*ms[0]; }
      //   if(hasRt) { H0[O0[y]+hb] += M0[y]*ms[3]; }
      // } else for( ; ; y++ ) {
      //   yb0 = (int) yb; if(yb0>=hb-1) break; GHinit;
      //   _m0=sse::SET(M0[y]); _m1=sse::SET(M1[y]);
      //   if(hasLf) { _m=sse::SET(0,0,ms[1],ms[0]);
  ...
  int nOrients, int softBin, float clip )
{
  ...
  wrFree(N); 
  // std::cout<<"wrFree(N)"<<std::endl;
  wrFree(R1); 
  // std::cout<<"wrFree(R1)"<<std::endl;
  wrFree(R2);
  // std::cout<<"wrFree(R2)"<<std::endl;
}
...
cv::Mat fhog(const cv::Mat& input, int binSize, int nOrients, float clip, bool crop){
    ...
    cv::Mat mid;
    input.convertTo(mid,CV_32F);
    mid = mid/255.f;
    mid = mid.t();
    float *I = (float*)(mid.data);
